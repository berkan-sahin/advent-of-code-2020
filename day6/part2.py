...
def questions_person_ex(answers: str, prev_yes_list: list):
    yes_list = [False for i in range(0, 26)]
    for ans in answers:
        yes_list[ord(ans) - ord("a")] = prev_yes_list[ord(ans) - ord("a")]
...
def questions_group(grp_answers):
    ...
    if type(grp_answers) == str:
        ...
        for i, person in enumerate(grp_answers):
            person = person[:-1]
            if i == 0: yes_list = questions_person(person, yes_list)
            else: yes_list = questions_person_ex(person, yes_list)
